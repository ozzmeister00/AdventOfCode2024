...
to the North Pole; a group of Senior
...
figure the Chief Historian must be in
one of the first fifty places they'll look, so in order to save Christmas, you 
need to help them get fifty stars on
...
hit a problem: their list of locations
to check is currently empty. Eventually, someone decides that the best place to
check first would be the Chief Historian's office.

Upon pouring into the office, everyone confirms that the Chief Historian is 
indeed nowhere to be found. Instead,
the Elves discover an assortment of notes and lists of historically significant 
locations! This seems to be the
 planning the Chief Historian was doing before he left. Perhaps these notes can 
be used to determine which locations
  to search?

Throughout the Chief's office, the historically significant locations are listed
 not by name but by a unique number
called the location ID. To make sure they don't miss anything, The Historians 
...
input), it quickly becomes clear that
...
right list, then the second-smallest left
...
For example, if you pair up a 3 from the left list with a 7 from the right list,
 the distance apart is 4; if you
...
The second-smallest number in the left list is 2, and the second-smallest number
 in the right list is another 3.
...
distances between all of the pairs you
 found. In the example above, this is 2 + 1 + 0 + 1 + 2 + 5, a total distance of
 11!
...
class Solver(solver.solver.ProblemSolver):
    ...
    def ProcessInput(self) -> list[tuple[int, int]]:
        """
        ...
        """
        processed = []
        for line in self.rawData.splitlines(keepends=False):
            x, y = line.split('   ')
    ...
    def SolvePartOne(self) -> int:
        """
        Pair up the smallest numbers in the left and right lists, and find the 
difference
        between each value

        :returns int total distance between the left and right lists:
        """

        left = []
        right = []

        for x, y in self.processed:
            left.append(x)
            right.append(y)
        ...
        for i, x in enumerate(left):
            distances.append(abs(x - right[i]))
    ...
    def SolvePartTwo(self):
        ...
        :return int a similarity score for the list of location IDs:
        """
        left = []
        right = collections.defaultdict(int)  # key = number, value = instances
