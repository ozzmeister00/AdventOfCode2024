...
The robot has no problem navigating the ship and finding the numeric keypad, but
 it's not designed for button pushing: it can't be told to push a specific 
...
pushing. In particular, if a robot arm is ever aimed at a gap where no button is
 ...
 So, don't do that. All robots will initially aim at the keypad's A key, 
wherever it is.
...
class Solver(solver.solver.ProblemSolver):
    def __init__(self, rawData=None):
        super(Solver, self).__init__(21, rawData=rawData)
